    ...
    internal class Demo8 : Demo7
    {
        ...
        public static void RunDemo8(string baseUrl, string password, string provisionedUser, string agentId, string channelId)
        {
            ...
            while (true)
            {
                ...
                messageIndex++;
                var messageParts = new dynamic[]
                {
                    plainTextMessagePart, hyperlinkMessagePart, channelLinkMessagePart, hashtagMessagePart,
                    codeBlockMessagePart
                };

                GetResponse(baseUrl + "/Collaboration/v1/Channels/" + HttpUtility.UrlEncode(channelId) + "/Messages",
                    token, new { IsAlert = false, MessageParts = messageParts });
